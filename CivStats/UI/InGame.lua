...
-- the only change is this CivStats.lua include
include( "CivStats" );
...
function DoCustomPingHandler(wParam, lParam)
	local plot = Map.GetPlot( UI.GetMouseOverHex() );
	if (plot ~= nil) then
		local notificationTitle = 'Ping!'
		local notificationText = "Pinged the plot at (" .. plot:GetX() .. ", " .. plot:GetY() .. ')';
		--Players[Game.GetActivePlayer()]:AddNotification(NotificationTypes.NOTIFICATION_EXPLORATION_RACE, text, "heading", plot:GetX(), plot:GetY(), FeatureTypes.FEATURE_EL_DORADO);
		Players[Game.GetActivePlayer()]:AddNotification(NotificationTypes.NOTIFICATION_GENERIC, notificationText, notificationTitle, plot:GetX(), plot:GetY());
	end
...
InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_PING][MouseEvents.LButtonUp] = DoCustomPingHandler;
InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_PING][MouseEvents.RButtonUp] = DoCustomPingHandler;
InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_PING][MouseEvents.PointerUp] = DoCustomPingHandler;
...
local NewInterfaceModeChangeHandler =
{
	...
	--[InterfaceModeTypes.INTERFACEMODE_PING] = DoCustomPingHandler,
